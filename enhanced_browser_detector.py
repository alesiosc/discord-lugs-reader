...
class DiscordBrowserDetector:
    def __init__(self):
        ...
        self.ocr = PaddleOCR(use_textline_orientation=True, lang='en')  # Updated to use correct parameter
        ...
        try:
            ...
            if result is not None:
                ...
                if isinstance(result, list):
                    ...
                    logger.info(f"OCR result has {len(result)} elements")
                    for i, item in enumerate(result):
                        if item is not None:
                            ...
                            if hasattr(item, 'rec_texts'):
                                ...
                                logger.info(f"  Found {len(item.rec_texts)} text parts in item {i}")
                                ...
                                for j, subitem in enumerate(item):
                                    if isinstance(subitem, list) and len(subitem) > 1 and isinstance(subitem[1], list) and len(subitem[1]) > 0:
                                        ...
                                        logger.info(f"  Subitem {j}: {subitem[1][0]}")
            ...
            logger.info(f"PaddleOCR extracted text length: {len(text)} characters")
            
            # Print first part of extracted text for debugging (first 500 characters)
            logger.info(f"Extracted text preview: {text[:500]}...")
            
            # Define patterns to match ticker data - More flexible pattern to handle variations
            # Looking for patterns like: [NQ] Published Level: Lug Timestamp: 11/4/2025 4:55:00 AM, Mid: 25694.5, Lower: 25524.75, Upper: 25864.25
            ...
            # If no matches found with primary pattern, try a more flexible one
            matches = re.findall(pattern, text, re.IGNORECASE | re.DOTALL)
            logger.info(f"Found {len(matches)} ticker matches with primary pattern")
            
            if not matches:
                # More flexible pattern that looks for the general structure
            ...
            # Get the latest for each type (NQ, ES, YM)
            latest_by_type = {}
            for item in ticker_data:
                ticker_type = item['type']
                if ticker_type in ['NQ', 'ES', 'YM', 'RTY', 'CL']:  # Added RTY and CL support
    ...
    def format_and_randomize_data(self, ticker_data):
        ...
        for item in ticker_data:
            ...
            elif item['type'] == 'RTY':  # Added RTY with ±3 range
                randomized_mid += random.uniform(-3, 3)
                randomized_lower += random.uniform(-3, 3)
                randomized_upper += random.uniform(-3, 3)
